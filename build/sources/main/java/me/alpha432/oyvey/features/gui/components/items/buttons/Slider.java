...
public class Slider
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y, ((Number) this.setting.getValue(true)).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4f) * this.partialMultiplier(), this.y + (float) this.height - 0.5f, !this.isHovering(mouseX, mouseY) ? OyVey.colorManager.getColorWithAlpha(OyVey.moduleManager.getModuleByClass(ClickGui.class).hoverAlpha.getValue(true)) : OyVey.colorManager.getColorWithAlpha(OyVey.moduleManager.getModuleByClass(ClickGui.class).alpha.getValue(true)));
        OyVey.textManager.drawStringWithShadow(this.getName() + " " + ChatFormatting.WHITE + (this.setting.getValue(true) instanceof Float ? this.setting.getValue(true) : Double.valueOf(((Number) this.setting.getValue(true)).doubleValue())), this.x + 2.3f, this.y - 1.7f - (float) OyVeyGui.getClickGui().getTextOffset(), -1);
    }
