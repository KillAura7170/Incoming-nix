...
public class AutoArmor
    ...
    private final Setting<Integer> closestEnemy = this.register(new Setting<Object>("Enemy", 8, 1, 20, v -> this.mendingTakeOff.getValue(true)));
    private final Setting<Integer> repair = this.register(new Setting<Object>("Repair%", 80, 1, 100, v -> this.mendingTakeOff.getValue(true)));
    ...
    public void onTick() {
        ...
        if (this.timer.passedMs((int) ((float) this.delay.getValue(true) * OyVey.serverManager.getTpsFactor()))) {
            if (!this.taskList.isEmpty()) {
    ...
    private void getSlotOn(int slot, int target) {
        if (this.taskList.isEmpty()) {
            this.doneSlots.remove((Integer) target);
            this.taskList.add(new InventoryUtil.Task(target));
            this.taskList.add(new InventoryUtil.Task(slot));
            this.taskList.add(new InventoryUtil.Task());
        }
