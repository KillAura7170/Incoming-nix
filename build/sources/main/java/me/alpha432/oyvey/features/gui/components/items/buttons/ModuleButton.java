...
public class ModuleButton
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (!this.items.isEmpty()) {
            if (HUD.getInstance().future.getValue(true)) {
                mc.getTextureManager().bindTexture(this.logo);
                ModuleButton.drawCompleteImage(this.x - 4.0f + (float) this.width - 7.4f, this.y - 2.2f - (float) OyVeyGui.getClickGui().getTextOffset(), 10, 10);
            }
            if (this.subOpen) {
                float height = 1.0f;
