...
public class ConfigManager implements Util {
    ...
    public String config = "Incoming/config/";
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("Incoming").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("Incoming/" + name + "/"))) {
            this.config = "Incoming/" + name + "/";
        } else {
            this.config = "Incoming/config/";
        }
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("Incoming").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("Incoming/" + name + "/"));
    }
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("Incoming/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("Incoming", ""));
                writer.close();
            } else {
