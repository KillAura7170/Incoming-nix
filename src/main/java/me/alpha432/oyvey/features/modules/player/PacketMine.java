...
public class PacketMine extends Module {
    ...
    public PacketMine() {
        ...
        this.red = (Setting<Integer>) this.register(new Setting("Red", 125, 0, 255, v -> this.render.getValue(true)));
        this.green = (Setting<Integer>) this.register(new Setting("Green", 105, 0, 255, v -> this.render.getValue(true)));
        this.blue = (Setting<Integer>) this.register(new Setting("Blue", 255, 0, 255, v -> this.render.getValue(true)));
        this.box = (Setting<Boolean>) this.register(new Setting("Box", false, v -> this.render.getValue(true)));
        this.outline = (Setting<Boolean>) this.register(new Setting("Outline", true, v -> this.render.getValue(true)));
        this.lineWidth = (Setting<Float>) this.register(new Setting("LineWidth", 1.0f, 0.1f, 5.0f, v -> this.outline.getValue(true) && this.render.getValue(true)));
        this.boxAlpha = (Setting<Integer>) this.register(new Setting("BoxAlpha", 85, 0, 255, v -> this.box.getValue(true) && this.render.getValue(true)));
        this.breakTime = -1.0f;
    ...
    public void onUpdate() {
        ...
        if (this.noDelay.getValue(true)) {
            PacketMine.mc.playerController.blockHitDelay = 0;
        }
        if (this.isMining && this.lastPos != null && this.lastFacing != null && this.noBreakAnim.getValue(true)) {
        ...
        if (this.reset.getValue(true) && PacketMine.mc.gameSettings.keyBindUseItem.isKeyDown() && !this.allow.getValue(true)) {
    ...
    public void onRender3D(final Render3DEvent event) {
        if (this.render.getValue(true) && this.currentPos != null) {
            final Color color = new Color(this.red.getValue(true), this.green.getValue(true), this.blue.getValue(true), this.boxAlpha.getValue(true));
            RenderUtil.gradientBox(this.currentPos, color, this.lineWidth.getValue(true), this.outline.getValue(true), this.box.getValue(true), this.boxAlpha.getValue(true), true);
        }
    }
    ...
    public void onPacketSend(final PacketEvent.Send event) {
        ...
        if (event.getStage() == 0) {
            if (this.noSwing.getValue(true) && event.getPacket() instanceof CPacketAnimation) {
                event.setCanceled(true);
            }
            if (this.noBreakAnim.getValue(true) && event.getPacket() instanceof CPacketPlayerDigging) {
    ...
    public void onBlockEvent(final BlockEvent event) {
        ...
        if (event.getStage() == 3 && this.reset.getValue(true) && PacketMine.mc.playerController.curBlockDamageMP > 0.1f) {
            PacketMine.mc.playerController.isHittingBlock = true;
        }
        if (event.getStage() == 4 && this.tweaks.getValue(true)) {
            ...
            if (this.doubleBreak.getValue(true)) {
