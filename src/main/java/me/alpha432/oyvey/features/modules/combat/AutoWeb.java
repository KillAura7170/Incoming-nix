...
public class AutoWeb
    ...
    private List<Vec3d> getPlacements() {
        ...
        if (this.lowerbody.getValue(true)) {
            list.add(baseVec);
        }
        if (this.upperBody.getValue(true)) {
    ...
    private boolean check() {
        ...
        if (this.disable.getValue(true) && !this.startPos.equals(EntityUtil.getRoundedBlockPos(AutoWeb.mc.player))) {
            this.disable();
            return true;
        }
        if (obbySlot == -1) {
