            ...
            switch (commands[0]) {
                case "add": {
                    ...
                    if (FriendSettings.getInstance().notify.getValue(true)) {
                        mc.player.connection.sendPacket(new CPacketChatMessage("/w " + commands[1] + " I just added you to my friends list on Incoming!"));
                    }
                    return;
                }
                case "del": {
                    ...
                    if (FriendSettings.getInstance().notify.getValue(true)) {
                        mc.player.connection.sendPacket(new CPacketChatMessage("/w " + commands[1] + " I just removed you from my friends list on Incoming!"));
                    }
